...
- Evidence Harvest must create a deterministic Coverage Inventory for every covered source root before selecting bounded review inputs. The inventory must count eligible files after exclusions and either list them directly or reference a run artifact that lists them. Candidate files are a review subset, not a substitute for this inventory.
- In `full` mode, missing or empty coverage counts are invalid when a covered root contains eligible files. A zero count is valid only when the inventory proves the root has no eligible files and records that reason. Do not declare full discovery complete from hand-picked candidate files without an inventory-backed omission audit.
- Evidence Harvest must sweep utility-heavy namespaces present in the Coverage Inventory before shard planning. For Java, include paths or packages named `util`, `utils`, `helper`, `common`, `core`, and `reflect`; for TypeScript or JavaScript, include shared `util`, `utils`, `helpers`, `composables`, and `shared` modules. These are review signals, not automatic acceptance decisions.
...
- Shard plans, chunk plans, reviewed chunks, and shard aggregates must carry coverage accounting that reconciles reviewed, excluded, deferred, and unreviewed eligible files back to the Coverage Inventory.
...
7. Catalog Finalizer Worker: use stronger reasoning, inspect source anchors directly, own semantic decisions, run a mandatory gap audit against the Coverage Inventory, deduplicate entries, decide catalog entries, class, module, or origin-level suppressions, run-local deferrals, and blocking questions, assign final capability tags, descriptions, and Artifact Priority values, and write the entry-centric Discovery Decision File without deferrals. The gap audit must review utility-heavy namespaces and project-owned imports from accepted artifacts; an imported utility-like class or module that is neither reviewed nor represented by an entry, suppression, or documented run-local deferral is a gap finding. Capability tags must describe the accepted utility's actual reuse domain; reflection utilities should use canonical reflection-oriented tags such as `reflection`, `introspection`, `method-access`, or `field-access` when applicable.
8. Decision Review Worker: review the Discovery Decision File and upstream artifacts before apply, checking Coverage Inventory reconciliation, missed-scan risk, utility-namespace coverage, project-owned import gap findings, origin normalization, project-owned priority dominance, external usage-count priority, and support for project-owned-first consult results with a default limit of 5 and a maximum limit of 10. It must return repair needed rather than pass when a `full` run has missing coverage counts, unjustified zero counts, unreconciled eligible files, or only manually curated candidates. It must not modify the Discovery Decision File, and returns pass, repair needed, or blocking decision needed.
...
- A `full` run must not reach apply until every covered root reconciles to its Coverage Inventory and Decision Review confirms that utility-heavy namespaces and project-owned utility imports were audited for omissions.
