...
8. After terminal implementation, dispatch review and repair workers when required.
9. Write back every implementation, review, repair, skipped, blocked, failed, or completed result.
10. Finish only when no runnable worker remains active and all downstream blockers are recorded.

## Worker Rules

- One implementation worker owns one runnable issue.
- Workers must not revert user or other-agent changes.
- Workers must use the issue shared channel for progress, blockers, and terminal reports.
...
See [REFERENCE.md](REFERENCE.md) for dispatch payload rules, supervision, abnormal stops,
issue write-back, review/repair loops, failure handling, ready-queue scheduling,
progress updates, and final handoff format.
